...
sf::Vector2f ants::rectCenter(const sf::FloatRect &rect) {
	return sf::Vector2f(rect.left + rect.width * 0.5f, rect.top + rect.height * 0.5f);
}

bool ants::rectIntersects(const sf::FloatRect &rect, const sf::FloatRect &other) {
...
bool ants::rectContains(const sf::FloatRect &rect, const sf::FloatRect &other) {
...
sf::Vector2f ants::rectHalfDims(const sf::FloatRect &rect) {
	return sf::Vector2f(rect.width * 0.5f, rect.height * 0.5f);
}

sf::Vector2f ants::rectDims(const sf::FloatRect &rect) {
	return sf::Vector2f(rect.width, rect.height);
}

sf::Vector2f ants::rectLowerBound(const sf::FloatRect &rect) {
	return sf::Vector2f(rect.left, rect.top);
}

sf::Vector2f ants::rectUpperBound(const sf::FloatRect &rect) {
	return sf::Vector2f(rect.left + rect.width, rect.top + rect.height);
}

sf::FloatRect ants::rectFromBounds(const sf::Vector2f &lowerBound, const sf::Vector2f &upperBound) {
	return sf::FloatRect(lowerBound.x, lowerBound.y, upperBound.x - lowerBound.x, upperBound.y - lowerBound.y);
}

float ants::vectorMagnitude(const sf::Vector2f &vector) {
	return std::sqrt(vector.x * vector.x + vector.y * vector.y);
}

float ants::vectorMagnitudeSquared(const sf::Vector2f &vector) {
	return vector.x * vector.x + vector.y * vector.y;
}

sf::Vector2f ants::vectorNormalize(const sf::Vector2f &vector) {
...
float ants::vectorProject(const sf::Vector2f &left, const sf::Vector2f &right) {
	assert(vectorMagnitudeSquared(right) != 0.0f);

	return vectorDot(left, right) / vectorMagnitudeSquared(right);
}

sf::FloatRect ants::rectRecenter(const sf::FloatRect &rect, const sf::Vector2f &center) {
	sf::Vector2f dims = rectDims(rect);

	return sf::FloatRect(center - rectHalfDims(rect), dims);
}

float ants::vectorDot(const sf::Vector2f &left, const sf::Vector2f &right) {
	return left.x * right.x + left.y * right.y;
}

sf::FloatRect ants::rectExpand(const sf::FloatRect &rect, const sf::Vector2f &point) {
...
bool ants::shapeIntersection(const sf::ConvexShape &left, const sf::ConvexShape &right) {
...
sf::ConvexShape ants::shapeFromRect(const sf::FloatRect &rect) {
...
sf::ConvexShape ants::shapeFixWinding(const sf::ConvexShape &shape) {
...
bool ants::rayIntersect(const sf::Vector2f &as, const sf::Vector2f &ad, const sf::Vector2f &bs, const sf::Vector2f &bd, sf::Vector2f &intersection) {
	float dx = bs.x - as.x;
	float dy = bs.y - as.y;
